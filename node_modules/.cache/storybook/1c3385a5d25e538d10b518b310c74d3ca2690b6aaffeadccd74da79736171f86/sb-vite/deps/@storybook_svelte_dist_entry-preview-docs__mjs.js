...
var parseTypeToControl = (type) => {
  ...
  else if (type.kind === "union" && Array.isArray(type.type) && !type.type.find((t) => t.type !== "string")) return { type: "radio", options: type.type.filter((t) => t.kind === "const").map((t) => t.value) };
  return null;
};
